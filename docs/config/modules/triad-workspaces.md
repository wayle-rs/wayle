...
Triad workspace indicators with click-to-switch.

Add it to your layout with `triad-workspaces`:

```toml
[[bar.layout]]
monitor = "*"
right = ["triad-workspaces"]
```
...
|---|---|---|---|
| `min-workspace-count` | u8 | `0` | Always-visible ceiling for existing workspaces by index. |
...
| `left-click` | [`TriadWorkspaceClickAction`](/config/types#triad-workspace-click-action) | `"focus:this"` | Action on left click. |
| `middle-click` | [`TriadWorkspaceClickAction`](/config/types#triad-workspace-click-action) | `""` | Action on middle click. |
| `right-click` | [`TriadWorkspaceClickAction`](/config/types#triad-workspace-click-action) | `""` | Action on right click. |
| `scroll-up` | [`TriadWorkspaceClickAction`](/config/types#triad-workspace-click-action) | `"focus:previous"` | Action on scroll up. |
| `scroll-down` | [`TriadWorkspaceClickAction`](/config/types#triad-workspace-click-action) | `"focus:next"` | Action on scroll down. |

::: details More about `min-workspace-count`

Existing workspaces at or below this index stay visible even when
empty. When `0` (default), no extra workspaces are forced visible.
...
```toml
[modules.triad-workspaces.workspace-map]
...
```toml
[modules.triad-workspaces.app-icon-map]
