//! Live CSS provider applied to each `ExtWorkspaces` component instance.
...
pub(super) fn apply_styling(
    ...
    let inactive_color = resolve_color(&ws_config.inactive_color, is_wayle_theme);
    ...
    let mut css = format!(
        ".workspaces.ext {{ \
            ...
            --ws-inactive-color: {inactive_color}; \
    ...
    for (key, style) in &ws_config.workspace_map.get() {
        ...
        for selector_class in classes_for_key(key) {
            css.push_str(&format!(
                ".workspaces.ext .workspace.{selector_class} {{ --ws-override-color: {color_css}; }}"
            ));
        }
    }
...
///
/// Numeric keys generate both `ws-id-N` (local object-id key) and
/// `ws-name-N` (numeric workspace name) so users can key by either without
/// picking the right one. Non-numeric keys only generate `ws-name-K`.
