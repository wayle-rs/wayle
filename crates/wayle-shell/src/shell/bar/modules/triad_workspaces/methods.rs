//! [`TriadWorkspaces`] private impl methods: rebuild, border classes, action dispatch.
...
use wayle_config::{
    ...
    schemas::{
        bar::BorderLocation,
        modules::{
            ActiveIndicator, DisplayMode, TriadLabelStrategy as LabelStrategy,
            TriadWorkspaceClickAction as WorkspaceClickAction, TriadWorkspaceMap, UrgentMode,
        },
    },
};
use wayle_triad::Window;

use super::{
    BLINK_INTERVAL, TriadWorkspaces, TriadWorkspacesCmd,
    button::{AppIconInit, TriadWorkspaceButtonInit},
...
impl TriadWorkspaces {
    ...
    pub(super) fn rebuild_buttons(&mut self) {
        let workspaces = self.triad.workspaces.get();
        let windows = self.triad.windows.get();
        let config = self.config.config();
        let ws_config = &config.modules.triad_workspaces;
        ...
        let snapshots = workspaces
            ...
            .map(|workspace| WorkspaceSnapshot {
                ...
                is_focused: workspace.is_active.get(),
                is_output_visible: workspace.is_output_visible.get(),
                has_windows: workspace.occupied.get(),
            })
            .collect();
    ...
    pub(super) fn sync_blink(&mut self, sender: &ComponentSender<TriadWorkspaces>) {
    ...
    pub(super) fn start_blink_timer(&mut self, sender: &ComponentSender<TriadWorkspaces>) {
        ...
        sender.command(move |out, shutdown| async move {
            ...
            loop {
                tokio::select! {
                    ...
                    _ = interval.tick() => {
                        let _ = out.send(TriadWorkspacesCmd::BlinkTick);
                    }
                }
            }
        });
    ...
    pub(super) fn dispatch_click_action(&self, action: WorkspaceClickAction, click_id: u64) {
        match action {
            ...
            WorkspaceClickAction::FocusNext => {
                self.spawn_focus_next();
            }
            WorkspaceClickAction::FocusPrevious => {
                self.spawn_focus_previous();
            }
    ...
    fn spawn_focus_id(&self, id: u64) {
        let triad = self.triad.clone();
        tokio::spawn(async move {
            if let Err(err) = triad.focus_tag(id).await {
                warn!(error = %err, workspace_id = id, "triad focus_tag failed");
            }
        });
    }
    ...
    fn spawn_focus_next(&self) {
        let triad = self.triad.clone();
        tokio::spawn(async move {
            if let Err(err) = triad.focus_tag_right().await {
                warn!(error = %err, action = "focus-tag-right", "triad action failed");
            }
        });
    }
    ...
    fn spawn_focus_previous(&self) {
        let triad = self.triad.clone();
        tokio::spawn(async move {
            if let Err(err) = triad.focus_tag_left().await {
                warn!(error = %err, action = "focus-tag-left", "triad action failed");
            }
        });
    }
}
...
struct ButtonLayout {
    ...
    workspace_map: TriadWorkspaceMap,
    blink_on: bool,
}
...
fn any_urgent(displayed: &[WorkspaceSnapshot], windows: &HashMap<u64, Arc<Window>>) -> bool {
    ...
    displayed.iter().any(|snapshot| snapshot.is_urgent)
}
...
fn windows_on_workspace(
    ...
    let mut result: Vec<Arc<Window>> = windows
        .values()
        .filter(|window| window.tag_id.get() == Some(workspace_id))
        .cloned()
        .collect();

    result.sort_by_key(|window| {
        let pos = window.position.get();
        let primary = pos.column_idx.unwrap_or(u32::MAX);
        let secondary = pos.window_idx.unwrap_or(u32::MAX);
