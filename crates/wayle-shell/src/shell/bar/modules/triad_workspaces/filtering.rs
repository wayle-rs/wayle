...
//!
//! Empty workspaces (no windows, not active, not output-visible) are hidden by
//! default. `min-workspace-count` includes existing workspaces whose index is
//! at or below that ceiling. No placeholder workspaces are fabricated.
...
pub(super) fn collect_displayed(
    ...
    let (mut shown, hidden): (Vec<_>, Vec<_>) = candidates.into_iter().partition(|snapshot| {
        snapshot.has_windows || snapshot.is_active || snapshot.is_output_visible
    });

    for snapshot in hidden {
        if usize::try_from(snapshot.idx).is_ok_and(|idx| idx <= ctx.min_workspace_count) {
            shown.push(snapshot);
        }
    }
...
fn sort_key(snapshot: &WorkspaceSnapshot) -> (String, u32, u64) {
    let output = snapshot.output.clone().unwrap_or_default();
    (output, snapshot.idx, snapshot.id)
}
...
mod tests {
    ...
    fn occupied(id: u64, idx: u32, output: &str) -> WorkspaceSnapshot {
        WorkspaceSnapshot {
            id,
            idx,
            name: None,
    ...
    fn named_empty(id: u64, idx: u32, output: &str, name: &str) -> WorkspaceSnapshot {
        WorkspaceSnapshot {
            id,
            idx,
    ...
    fn empty(id: u64, idx: u32, output: &str) -> WorkspaceSnapshot {
        WorkspaceSnapshot {
            id,
            idx,
            name: None,
    ...
    fn active_empty(id: u64, idx: u32, output: &str) -> WorkspaceSnapshot {
        WorkspaceSnapshot {
            id,
            idx,
            name: None,
    ...
    #[test]
    fn min_uses_workspace_index_not_name() {
        let snapshots = vec![
            occupied(10, 1, "DP-1"),
