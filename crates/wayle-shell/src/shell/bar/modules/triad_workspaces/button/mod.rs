//! Per-workspace button used by the [`TriadWorkspaces`] factory.
//!
...
///
/// `window_ids` lists the triad window ids this icon represents — usually one,
/// or multiple when [`app-icons-dedupe`] is on.
///
/// [`app-icons-dedupe`]: wayle_config::schemas::modules::TriadWorkspacesConfig::app_icons_dedupe
#[derive(Debug, Clone)]
...
#[derive(Debug, Clone)]
pub(crate) struct TriadWorkspaceButtonInit {
    pub id: u64,
...
pub(crate) struct TriadWorkspaceButton {
    pub(super) id: u64,
...
#[derive(Debug)]
pub(crate) enum TriadWorkspaceButtonInput {}

#[derive(Debug)]
pub(crate) enum TriadWorkspaceButtonOutput {
    LeftClick(u64),
    MiddleClick(u64),
    RightClick(u64),
    ScrollUp,
...
impl FactoryComponent for TriadWorkspaceButton {
    type Init = TriadWorkspaceButtonInit;
    type Input = TriadWorkspaceButtonInput;
    type Output = TriadWorkspaceButtonOutput;
    type CommandOutput = ();
    ...
    view! {
        ...
        gtk::Button {
            ...
            connect_clicked[sender, id = self.id] => move |_| {
                let _ = sender.output(TriadWorkspaceButtonOutput::LeftClick(id));
            },
...
fn attach_middle_click(
    button: &gtk::Button,
    sender: &FactorySender<TriadWorkspaceButton>,
    id: u64,
) {
    ...
    gesture.connect_released(move |_, _, _, _| {
        let _ = sender.output(TriadWorkspaceButtonOutput::MiddleClick(id));
    });
    button.add_controller(gesture);
}

fn attach_right_click(button: &gtk::Button, sender: &FactorySender<TriadWorkspaceButton>, id: u64) {
    let gesture = gtk::GestureClick::new();
    gesture.set_button(gtk::gdk::BUTTON_SECONDARY);
    let sender = sender.clone();
    gesture.connect_released(move |_, _, _, _| {
        let _ = sender.output(TriadWorkspaceButtonOutput::RightClick(id));
    });
    button.add_controller(gesture);
}

fn attach_scroll(button: &gtk::Button, sender: &FactorySender<TriadWorkspaceButton>) {
    ...
    controller.connect_scroll(move |_, _dx, dy| {
        if dy > 0.0 {
            let _ = sender.output(TriadWorkspaceButtonOutput::ScrollDown);
        } else if dy < 0.0 {
            let _ = sender.output(TriadWorkspaceButtonOutput::ScrollUp);
        }
