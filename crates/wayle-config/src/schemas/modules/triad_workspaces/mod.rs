//! Triad workspace switcher configuration.
...
impl JsonSchema for WorkspaceClickAction {
    fn schema_name() -> std::borrow::Cow<'static, str> {
        std::borrow::Cow::Borrowed("TriadWorkspaceClickAction")
    }

    fn json_schema(_generator: &mut schemars::SchemaGenerator) -> schemars::Schema {
        schemars::json_schema!({
            "description": "Click/scroll action: focus:this | focus:next | focus:previous | dropdown:NAME | shell command | empty for none",
            "type": "string"
        })
    }
}
...
/// [`TriadWorkspacesConfig::display_mode`] and the active/occupied/empty
/// color knobs.
///
/// ## Example
///
...
/// [modules.triad-workspaces.workspace-map.5]
...
#[wayle_config(i18n_prefix = "settings-modules-triad-workspaces")]
pub struct TriadWorkspacesConfig {
    /// Always-visible ceiling for existing workspaces by index.
    ///
...
impl ModuleInfoProvider for TriadWorkspacesConfig {
    fn module_info() -> ModuleInfo {
        ModuleInfo {
            name: String::from("triad-workspaces"),
            schema: || schema_for!(TriadWorkspacesConfig),
            layout_id: Some(String::from("triad-workspaces")),
            array_entry: false,
        }
    }
...
crate::register_module!(TriadWorkspacesConfig);
